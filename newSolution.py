...
LastEditTime: 2026-07-20 18:59:19
...
if commitCount < 2:  # 直接本地merge，即不是rebase又减少一次merge记录 | 这个merge大概不会产生冲突
    ...
    try:
        result = subprocess.run(
            ["gh", "pr", "view", '-R', 'LetMeFly666/LeetCode', str(prNumber), "--json", "title"],
            capture_output=True,
            text=True,
            check=True,
            encoding="utf-8",
    ...
    os.system(f'gh pr merge {prNumber}  -R LetMeFly666/LeetCode -s -d -t "{mergeTitle} (#{prNumber})"')
os.system(f'gh issue edit  -R LetMeFly666/LeetCode {issueNum} --remove-label "solving"')
