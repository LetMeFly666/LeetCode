/*
 * @Author: LetMeFly
 * @Date: 2025-10-25 20:12:40
 * @LastEditors: LetMeFly.xyz
 * @LastEditTime: 2025-10-25 20:15:50
 */
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    vector<pair<int, int>> dp(m);  // [<length, idx>, ...
    int ans = 0;
    for (int i = 0; i < m; i++) {
        int l = 0, idx = -1, choose = -1;
        for (int j = 0; j < i; j++) {
            if (b[j] < b[i] && (dp[j].first > l || dp[j].first == l && dp[j].second < idx)) {
