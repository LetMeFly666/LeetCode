<!--
 * @Author: LetMeFly
 * @Date: 2025-06-02 08:53:13
 * @LastEditors: LetMeFly.xyz
 * @LastEditTime: 2025-06-08 15:53:30
-->
# 方案构想
...
1. 校验位位置设计
    ...
    - 校验位固定位置可能导致字符串长度不足时越界
    - 校验位太密集会影响编码容量，太稀疏会增加误匹配概率


2. 字符大小关系生成不可逆

    - `s1[i]='a'`时无法生成更小的`s2[i]`来编码`1`
    - `s1[i]='z'`时无法生成更大的`s2[i]`来编码`0`
    - 必须确保字符在可调整范围内
