...
@redirect_blueprint.route('/encode', methods=['GET', 'POST'])
def encode_page():
    """编码页面"""
...
@redirect_blueprint.route('/redirect/', defaults={'path': ''})
@redirect_blueprint.route('/redirect/<path:path>')
...
#     # 尝试对 path 进行 base64 解码还原原始 target_url
#     try:
#         # 注意：Base64 编码可能去掉了末尾的 padding，需要补全
#         missing_padding = len(path) % 4
#         if missing_padding:
#             path += '=' * (4 - missing_padding)
#         decoded_bytes = base64.urlsafe_b64decode(path)
#         target_url = decoded_bytes.decode('utf-8')
#     except Exception as e:
#         return f"Base64 解码失败：{e}", 400
...
# @app.route('/admin')
@redirect_blueprint.route('/admin')
def admin_page():
    """管理后台"""
    # 创建表格HTML
    matched_html = """
