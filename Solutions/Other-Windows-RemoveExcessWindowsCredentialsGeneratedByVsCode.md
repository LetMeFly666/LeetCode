...
都2026年了，电脑上Github CLI `gh`版本还是2023年的`gh version 2.32.1 (2023-07-24)`，不支持[多账号](https://github.com/LetMeFly666/LeetCode/pull/1401)登录等一些特性，更换新版本`gh version 2.87.0 (2026-02-18)`后运行报错：
...
我64G的内存跑不了一个gh身份token的迁移？(bushi)，一搜发现Github Desktop([#15217](https://github.com/desktop/desktop/issues/15217))有类似的问题，[评论](https://github.com/desktop/desktop/issues/15217#issuecomment-1491979890)说删个旧凭据可能有效。

一看凭据，发现有大量的2022-2023年间VsCode存储的微软账号登录凭据，指定是当时设计的有点问题。

![大量VsCode存储的MS鉴权凭据的部分](https://cors.letmefly.xyz/https://i-blog.csdnimg.cn/direct/6c59a244603d43fb9ab75cff4c962e2a.png)
...
使用**记事本**另存为`xx.ps1`，编码为`ANSI`，管理员身份启动powershell，输入这个脚本的路径并回车，脚本会先列举出以`vscodevscode.`开头的凭据，按`Y`回车可以批量删除，世界开始变得清净了。
