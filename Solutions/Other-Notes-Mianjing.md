...
二者同时存在的话，`Cache-Control`的优先级更高。（推荐）
...
4. 若$min\_trx\_id\leq trx\_id \lt max\_trx\_id$：若该事务id在m_ids列表则说明生成该版本记录事务依然活跃着仍未提交(该版本对当前事务不可见)；若该事务id不在m_ids中则说明生成该版本记录的事务已经被提交(该版本记录对当前事务可见)
