...
对```i++```和```++i```的争论褒贬不一，不知从何时起（大概是学C的时候老师就是这么教的）我的习惯是在for循环中使用i++而不是++i
...
+ 在终端i++中，使用指令```g++ -O0 i++.cpp -o i++```进行编译
+ 在终端++i中，使用指令```g++ -O0 ++i.cpp -o ++i```进行编译
+ 在终端i++O2中，使用指令```g++ -O2 i++.cpp -o i++O2```进行编译
