...
**为什么规范要这样设计？** 出于隐私和安全保护。用户（`blog.letmefly.xyz` 上的页面）原本只想请求 `letmefly.xyz`，被重定向到 `web.letmefly.xyz` 并非用户主动选择。如果浏览器把原始 Origin 透传给重定向目标，恶意服务器可以通过重定向让浏览器带着受信任的 Origin 去请求特权 API，构成混淆代理攻击（[confused deputy attack](https://en.wikipedia.org/wiki/Confused_deputy_problem)）。
...
> 同步发文于我的[个人博客](https://blog.letmefly.xyz/)，(AI)创作不易，转载经作者同意后请附上[原文链接](https://blog.letmefly.xyz/2026/04/19/Other-Nginx-CORSRedirectOriginNull/)哦~
>
